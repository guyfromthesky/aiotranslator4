...
class ThemeCreator(ttk.Window):
    ...
    def export_user_themes(self):
        ...
        outpath = asksaveasfilename(
            initialdir="/",
            initialfile="my theme.txt",
            filetypes=[("Text", "*.txt")],
        )
        base_name = os.path.basename(outpath)
        dir_name = os.path.dirname(outpath)
        theme_name, ext = os.path.splitext(base_name)
        outpath = os.path.join(dir_name, theme_name + '.txt')
        ...
        if outpath:
            ...
            f.write('\n        "colors": {')
            for row in self.color_rows:
                f.write("            \"" + row.label["text"]  + "\": \"" + row.color_value + "\",")
                f.write('\n')
            f.write('\n        },')
            f.write('\n    },')
            f.close()
